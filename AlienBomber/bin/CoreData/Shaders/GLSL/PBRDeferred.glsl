...
void PS()
{
...
#else
	vec4
	depthInput  = texture2DProj(sDepthBuffer, vScreenPos);
#ifdef HWDEPTH
	float depth = ReconstructDepth(depthInput.r);
#else
	float depth = DecodeDepth(depthInput.rgb);
...
#else
	vec3
	worldPos = vFarRay * depth / vScreenPos.w;
#endif
	vec4
	albedoInput = texture2DProj(sAlbedoBuffer, vScreenPos);
	vec4
	normalInput = texture2DProj(sNormalBuffer, vScreenPos);
	vec4
	specularInput = texture2DProj(sSpecMap, vScreenPos);
#endif
	...
	vec3
	normal = normalInput.rgb;
	float roughness = length(normal);
	normal = normalize(normal);

	vec3
	specColor = specularInput.rgb;

	vec4
	projWorldPos = vec4(worldPos, 1.0);
	...
	vec3
	BRDF = GetBRDF(lightDir, lightVec, toCamera, normal, roughness, albedoInput.rgb, specColor);
