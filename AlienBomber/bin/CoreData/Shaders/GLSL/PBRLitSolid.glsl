...
void PS()
{
	...
	vec3
	specColor = mix(0.08 * cMatSpecColor.rgb, diffColor.rgb, metalness);
	...
	vec3
	toCamera = normalize(vWorldPos.xyz - cCameraPosPS);
	vec3
	reflection = normalize(reflect(toCamera, normal));

	vec3
	cubeColor = vVertexLight.rgb;
